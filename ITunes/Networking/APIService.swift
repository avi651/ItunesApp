...
class APIService {
    ...
    public func fetchAlbums(searchTerm: String, page: Int, limit: Int, completion: @escaping(Result<AlbumResult, APIError>) -> Void) {
    ...
    public func fetchAlbum(with albumID: Int, completion: @escaping(Result<AlbumResult,APIError>) -> Void) {
        let url = createURL(for: albumID, type: .album)
        fetch(type: AlbumResult.self, url: url, completion: completion)
    }
    
    public func fetchSongs(for albumID: Int, completion: @escaping(Result<SongResult,APIError>) -> Void) {
        let url = createURL(for: albumID, type: .song)
        fetch(type: SongResult.self, url: url, completion: completion)
    }
    
    public func fetchSongs(searchTerm: String, page: Int, limit: Int, completion: @escaping(Result<SongResult,APIError>) -> Void) {
        let url = createURL(for: searchTerm, type: .song, page: page, limit: limit)
        fetch(type: SongResult.self, url: url, completion: completion)
    }
    
    public func fetchMovies(searchTerm: String, completion: @escaping(Result<MovieResult,APIError>) -> Void) {
   ...
   private func fetch<T: Decodable>(type: T.Type, url: URL?, completion: @escaping(Result<T,APIError>) -> Void) {
    ...
    private func createURL(for searchTerm: String, type: EntityType, page: Int?, limit: Int?) -> URL? {
        let url = "\(APIConstant.baseURL)/search"
